...
export function ExamListView() {
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      AxiosComponent({
        method: 'delete',
        url: `${BASE_LMS_API}/exams/${DeleteItem[0].id}`,
        callback: (status, data) => {
          if (status) {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.exam.edit(id));
    },
    [router]
  );
  const handleExamQuestionRow = useCallback(
    (id) => {
      router.push(paths.dashboard.examQuestion.list(id));
    },
    [router]
  );
                      ...
                      {dataFiltered
                        ...
                        .map((row, index) => (
                          <ExamTableRow
                            ...
                            onExamQuestionRow={() => handleExamQuestionRow(row.id)}
                          />
                        ))}
