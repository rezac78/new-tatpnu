...
import { SubMenuTableRow } from '../examQuestion-table-row';
import { SubMenuTableToolbar } from '../examQuestion-table-toolbar';
...
export function ExamQuestionListView() {
  ...
  const getMenus = useCallback(async () => {
    await AxiosComponent({
      method: 'get',
      url: `${BASE_LMS_API}/exam-questions/index/${IdQus.id}`,
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      AxiosComponent({
        method: 'delete',
        url: `${BASE_LMS_API}/exam-questions/${DeleteItem[0].id}`,
        callback: (status, data) => {
          if (status) {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.examQuestion.edit(id));
    },
    [router]
  );
  const handleQuestionOptionsRow = useCallback(
    (id) => {
      router.push(paths.dashboard.questionOption.list(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.examQuestion.new(IdQus.id)}
              variant="contained"
                      ...
                      {dataFiltered
                        ...
                        .map((row, index) => (
                          <SubMenuTableRow
                            ...
                            onQuestionOptionsRow={() => handleQuestionOptionsRow(row.id)}
                          />
                        ))}
