...
import { UserProductsTableRow } from '../productSections-table-row';
import { UserTableFiltersResult } from '../user-table-filters-result';
import { UserProductsTableToolbar } from '../productSections-table-toolbar';
...
const TABLE_HEAD = [
  ...
  { id: 'StadyTime', label: 'تایم مطالعه' },
  { id: 'productTitle', label: 'فصل' },
  { id: 'type', label: 'نوع' },
  { id: 'options', width: 88 },
...
export function ProductSectionsListView() {
  ...
  const getUserProducts = useCallback(async () => {
    await AxiosComponent({
      method: 'get',
      url: `${BASE_LMS_API}/product-sections/admin/index/${IdParams.session_id}`,
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      AxiosComponent({
        method: 'delete',
        url: `${BASE_LMS_API}/product-sections/${DeleteItem[0].id}`,
        callback: (status, data) => {
          if (status) {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.productSections.edit(id, IdParams.session_id));
    },
    [IdParams.session_id, router]
  );
      ...
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: t.dashboard.pages.productSeasons, href: '#' },
            { name: t.dashboard.tableCommon.list },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.productSections.new(IdParams.product_id, IdParams.session_id)}
              variant="contained"
