...
import { ProfessorsTableRow } from '../professors-table-row';
import { ProfessorsTableToolbar } from '../professors-table-toolbar';
...
export function ProfessorsListView() {
  ...
  const getProducts = useCallback(async () => {
    await AxiosComponent({
      method: 'get',
      url: `${BASE_LMS_API}/professors/index/${IdTech.id}`,
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      AxiosComponent({
        method: 'delete',
        url: `${BASE_LMS_API}/professors/${DeleteItem[0].id}`,
        callback: (status, data) => {
          if (status) {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.professors.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.professors.new(IdTech.id)}
              variant="contained"
