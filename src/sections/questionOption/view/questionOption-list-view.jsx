...
import { QuestionOptionTableRow } from '../questionOption-table-row';
import { UserTableFiltersResult } from '../user-table-filters-result';
import { QuestionOptionTableToolbar } from '../questionOption-table-toolbar';
...
export function QuestionOptionListView() {
  const table = useTable();
  const t = fa;
  const IdQus = useParams();
  ...
  const getMenus = useCallback(async () => {
    await AxiosComponent({
      method: 'get',
      url: `${BASE_LMS_API}/question-options/index/${IdQus.id}`,
      callback: (status, data) => {
        if (status) {
          const tableDataNew = Object.keys(data.data[IdQus.id].options).map((key) => ({
            id: key,
            title: data.data[IdQus.id].options[key],
          }));
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      AxiosComponent({
        method: 'delete',
        url: `${BASE_LMS_API}/question-options/${DeleteItem[0].id}`,
        callback: (status, data) => {
          if (status) {
  ...
  const handleEditRow = useCallback(
    (id, editId) => {
      router.push(paths.dashboard.questionOption.edit(id, IdQus.id));
    },
    [IdQus.id, router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.questionOption.new(IdQus.id)}
              variant="contained"
