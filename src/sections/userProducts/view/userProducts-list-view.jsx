...
import { UserProductsTableRow } from '../userProducts-table-row';
import { UserTableFiltersResult } from '../user-table-filters-result';
import { UserProductsTableToolbar } from '../userProducts-table-toolbar';
...
const TABLE_HEAD = [
  ...
  { id: 'productTitle', label: 'نام محصول' },
  { id: 'type', label: 'نوع' },
  { id: 'options', width: 88 },
...
export function UserProductsListView() {
  ...
  const getUserProducts = useCallback(async () => {
    await AxiosComponent({
      method: 'get',
      url: `${BASE_LMS_API}/user-products`,
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      AxiosComponent({
        method: 'delete',
        url: `${BASE_LMS_API}/user-products/${deleteRow[0].id}`,
        callback: (status, data) => {
          if (status) {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.userProducts.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.userProducts.new}
              variant="contained"
